...
def get_bed_coords(chrom_reads, chrom_length, fragment_size, chrom, verbose):
    ...
    *Tags on different strands are positioned differently
    ...
    taglist = []
    scorelist = [] # ajoute par samira
    scoredictionnary = {} # ajoute par samira
    print_return = ""
    for read in chrom_reads:
        ...
        if (start < 0):
            ...
            if (strand == '+'):
                ...
                scoredictionnary[str(position)]=score# ajoute par samira
                taglist.append(position)
                #postive_tag_counts += 1
                postive_tag_counts += score # ajoute par samira
                ...
                negative_tag_counts += score # ajoute par samira
    ...
    return (taglist, print_return,scorelist,total_tag_counts) # ajoute par samira


def Generate_windows_and_count_tags(taglist, chrom, chrom_length, window_size,scorelist):  # modified by Samira 
    """
    ...
    if (len(taglist) > 0):
        ...
        tag_count_in_current_window = scorelist[0] # added by Samira 
        for i in range(1, len(taglist)):
            start = (taglist[i] // window_size) * window_size
            score = scorelist[i] # added by Samira
            if start == current_window_start:
                # tag_count_in_current_window += 1 
                tag_count_in_current_window += score # added by Samira 
...
def makeGraphFile(args, filtered, chrom, chrom_length):
    ...
    tag_list, print_return,scorelist, finalCount = get_bed_coords(chrom_reads, chrom_length, args.fragment_size, chrom, args.verbose)  # ajoute par Samira
    ...
    if finalCount != tag_count :  # ajoute par samira
        print("problem") 
